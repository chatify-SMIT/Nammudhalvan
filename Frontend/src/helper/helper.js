...
export async function getMessage({ from, to }) {
  try {
    const { data, status } = await axios.get("/api/getmsg", {
      params: { from, to },
    });
    return { data, status };
  } catch (error) {
...
export async function getsidedata(sender, receiver) {
  try {
    const response = await axios.post("/api/sidedata/", {
      sender: sender,
...
export async function setseen(sender, users) {
  try {
    console.log(sender, users);
    const response = await axios.post("/api/seen/", {
      sender: users,
      users: [sender],
    });
